...
def test_cost_function_wrappers_vars_and_err():
    for _ in range(20):
        ...
        cf1 = th.eb.VariableDifference(v1, cw1, t1)
        ...
        # Create wrappers check that they have the correct length
        objective.add(cf1)
        objective.add(cf2)
        objective._create_cost_function_wrappers()
        assert len(objective._cost_function_wrappers) == 2

        # Update weights after creating wrappers to see if data is picked up correctly
        # by the wrapper
        ...
        # Check that the wrappers have the right variables and error
        for cf in objective._cost_function_wrappers:
