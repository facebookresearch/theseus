...
def dataset():
    ...
    return {
        ...
        "ee_poses": torch.Tensor([pos + quat[3:] + quat[:3] for pos, quat in data["ee_poses"]]),
...
def test_forward_kinematics_seq(robot_model, dataset):
    ...
    for joint_state, ee_pose_target in zip(dataset["joint_states"], dataset["ee_poses"]):
        ee_se3_target = SE3(x_y_z_quaternion=ee_pose_target)
        ee_se3_computed = robot_model.forward_kinematics(joint_state)[ee_name]

        assert torch.allclose(ee_se3_target.between(ee_se3_computed).log_map(), torch.zeros(6))
...
def test_forward_kinematics_batched(robot_model, dataset):
    ...
    assert torch.allclose(ee_se3_target.between(ee_se3_computed).log_map(), torch.zeros(dataset["num_data"], 6))
