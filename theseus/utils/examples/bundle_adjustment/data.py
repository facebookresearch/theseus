...
class Camera:
    ...
    def to_params(self) -> List[float]:
        ...
        return [*R.numpy(), *t.numpy(), float(self.focal_length[0,0]),
                float(self.calib_k1[0,0]), float(self.calib_k2[0,0])]
    ...
    def from_params(params: List[float], name: str="Cam") -> "Camera":
        ...
        focal_length = th.Variable(data=torch.tensor([params[6:7]], dtype=torch.float64), name=name+"_focal_length")
        calib_k1 = th.Variable(data=torch.tensor([params[7:8]], dtype=torch.float64), name=name+"_calib_k1")
        calib_k2 = th.Variable(data=torch.tensor([params[8:9]], dtype=torch.float64), name=name+"_calib_k2")
...
class BundleAdjustmentDataset:
    ...
    def load_bal_dataset(path):
        ...
        with open(path, "rt") as out:
            ...
            for i in range(num_observations):
                ...
                feat = th.Point2(data=torch.tensor([
                    float(fields[2]), float(fields[3])
                ], dtype=torch.float64).unsqueeze(0), name=f"Feat{i}")
                observations.append(
                    Observation(camera_index=int(fields[0]),
            ...
            for i in range(num_points):
                ...
                for _ in range(3):
                    params.append(float(out.readline().rstrip()))
                points.append(th.Point3(data=torch.tensor(params, dtype=torch.float64).unsqueeze(0),
                        name=f"Pt{i}"))
    ...
    def load_from_file(path, gt_path: Optional[str]=None):
        ...
        return BundleAdjustmentDataset(cameras=cameras, points=points,
            observations=observations, gt_cameras=gt_cameras, gt_points=gt_points)

    def save_to_file(self, path, gt_path: Optional[str]=None):
        BundleAdjustmentDataset.save_bal_dataset(path, self.cameras, self.points, self.observations)
        if gt_path is not None:
            BundleAdjustmentDataset.save_bal_dataset(gt_path, self.gt_cameras, self.gt_points, self.observations)

    def histogram(self):
        buckets = np.zeros(11)
        for obs in self.observations:
            proj_pt = self.cameras[obs.camera_index].project_point(self.points[obs.point_index])
    ...
    def generate_synthetic(num_cameras: int,
        ...
        points = [
            th.Point3(data=gt_points[i].data + (torch.rand((1,3))*2-1) * 0.2,
        ...
        for i in range(num_cameras):
            ...
            for j in obs_pts:
                ...
                feat = gt_cameras[i].project_point(gt_points[j]) + (torch.rand(1,2)*2-1)*feat_random
        ...
        return BundleAdjustmentDataset(cameras, points, observations, gt_cameras, gt_points)
